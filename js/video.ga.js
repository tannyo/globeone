 ...
 * 2014-09-25 TKO Added support for "Completed video" by threshold.
 */
  ...
  vimeo = (function () {
    ...
    function start(iframe, options) {
      config = {
        ...
        complete: 90      // Property threshold for video completed.
      };
      ...
      if (options) {
        ...
        config.progress = (options.progress || config.progress);
        config.seek = (options.seek || config.seek);
        if (typeof options.complete === "number") {
          config.complete = options.complete;
        }
      }
    ...
    function stop() {
      ...
      if (!videoCompleted) {
        if (config.complete && timePercentComplete >= config.complete) {
          gaTrack('Completed video');
        } else {
          gaTrack('Closed video at ' + (timePercentComplete ? timePercentComplete : 0) + '%');
        }
      }
    }
